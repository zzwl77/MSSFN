import pandas as pd
from gazeheatplot import draw_heatmap2
import csv
import os
...
with tqdm(total=total_tasks, desc='Processing') as pbar:
    for task, task_range in task_ranges.items():
        for csv_file in csv_files:
            ...
            output_directory = os.path.join(os.path.dirname(csv_file), "heatstims")
            ...
            if task_data:
                ...
                # 分别绘制左右数据的热图
                draw_heatmap2(gaze_data_l, (1920, 1080), alpha=0.5, savefilename=output_name_l, imagefile=background_image, gaussianwh=200, gaussiansd=None)
                draw_heatmap2(gaze_data_r, (1920, 1080), alpha=0.5, savefilename=output_name_r, imagefile=background_image, gaussianwh=200, gaussiansd=None)
                pbar.update(1)